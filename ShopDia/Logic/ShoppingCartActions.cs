...
namespace ShopDia.Logic
{
    public class ShoppingCartActions : IDisposable
    {
        public string ShoppingCartId { get; set; }
        private AlbumContext _db = new AlbumContext();
        ...
        public void AddToCart(int id)
        {
            ...
            if (cartItem == null)
            {
                ...
                cartItem = new CartItem
                {
                    ...
                    Album = _db.Albums.SingleOrDefault(
               p => p.MaDia == id),
                    Quantity = 1,
       ...
       CartItemUpdates)
        {
            using (var db = new ShopDia.Models.AlbumContext())
            {
                try
                {
                    ...
                    foreach (var cartItem in myCart)
                    {
                        ...
                        {
                            if (cartItem.Album.MaDia == CartItemUpdates[i].BookId)
                            {
                                if (CartItemUpdates[i].PurchaseQuantity < 1 ||
                               CartItemUpdates[i].RemoveItem == true)
                                {
                                    RemoveItem(cartId, cartItem.MaDia);
                                }
                                else
                                {
                                    UpdateItem(cartId, cartItem.MaDia,
        ...
        {
            using (var _db = new ShopDia.Models.AlbumContext())
            {
                try
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == removeCartID && c.Album.MaDia ==
                                 removeBookID
        ...
        quantity)
        {
            using (var _db = new ShopDia.Models.AlbumContext())
            {
                ...
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == updateCartID && c.Album.MaDia ==
                                 updateBookID
                                  select c).FirstOrDefault();
                    if (myItem != null)
                    {
